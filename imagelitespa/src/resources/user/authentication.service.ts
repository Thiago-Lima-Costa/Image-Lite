import { AccessToken, Credentials, User, UserSesssionToken } from './users.resources';
...
class AuthService {
    ...
    initSession(token: AccessToken) {
        if(token.accessToken){
            ...
            const userSessionToken: UserSesssionToken = {
    ...
    setUserSession(userSessionToken: UserSesssionToken){
        localStorage.setItem(AuthService.AUTH_PARAM, JSON.stringify(userSessionToken));
    }

    getUserSession() : UserSesssionToken | null {
        ...
        const token: UserSesssionToken = JSON.parse(authString);
        return token;
    }

    isSessionValid() : boolean {
        const userSession: UserSesssionToken | null = this.getUserSession();
        if(!userSession){
            return false;
        }
